...
<Tabs>
  ...
  <Tab title="@pro/main">
    ...
    Each mode module calls `getArgs()` on startup and early-returns if its required flags are absent, so only one mode's logic runs per process invocation. The `backtest`, `live`, and `paper` modes each check for both `--entry` and their own mode flag (e.g. `--backtest`); `session` only checks `--session`.
