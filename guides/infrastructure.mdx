---
title: "Setting Up MongoDB and Redis Infrastructure"
...
<Steps>
  ...
  <Step title="Build all packages">
    ...
    Both scripts do the same thing: walk every directory under `packages/`, enter each one, and run `npm run build` inside it (which invokes `rollup -c`):
