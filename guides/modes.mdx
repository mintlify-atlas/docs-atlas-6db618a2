...
`strict: false` lets unknown flags (such as `--ui` and `--noCache` consumed by `@backtest-kit/cli`) pass through without throwing. `allowPositionals: true` lets the strategy file path appear as a bare positional argument at the end of the command.
...
<Tabs>
  ...
  <Tab title="Backtest B (Single)">
    ...
    npm run start -- --backtest --ui --noCache \
      ./content/apr_2026.strategy/apr_2026.strategy.ts
    ```
    No `--entry` flag — `backtest.ts` early-returns and `@backtest-kit/cli` runs a single-symbol backtest for whichever symbol the strategy file registers.
  </Tab>
  <Tab title="Live">
    ```bash
