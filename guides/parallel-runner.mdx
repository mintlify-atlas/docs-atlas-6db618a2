...
| Flag | Type | Purpose |
|---|---|---|
| `--backtest` | boolean | Activates the second guard in `backtest.ts` — without it the file is a no-op |
| `--entry` | boolean | Activates the first guard — distinguishes Mode A from Mode B |
| `--ui` | boolean | Starts the `@backtest-kit/ui` web interface on `:60050` (consumed by the CLI) |
| `--cache` | boolean | Triggers `CACHE_CANDLES_FN` before the runners start, pre-populating MongoDB with OHLCV candles |
